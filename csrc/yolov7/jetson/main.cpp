...
int main(int argc, char** argv)
{
    ...
    auto yolov7 = new YOLOv7(engine_file_path);
    yolov7->make_pipe(true);
    ...
    if (isVideo) {
        ...
        while (cap.read(image)) {
            objs.clear();
            yolov7->copy_from_Mat(image, size);
            auto start = std::chrono::system_clock::now();
            yolov7->infer();
            auto end = std::chrono::system_clock::now();
            yolov7->postprocess(objs);
            yolov7->draw_objects(image, res, objs, CLASS_NAMES, COLORS);
